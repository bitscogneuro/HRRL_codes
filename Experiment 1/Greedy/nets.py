...
class Net_J(nn.Module):
    ...
    def __init__(self, hyperparam: Hyperparam):
        ...
        self.norm1 = nn.LayerNorm(n_neurons)
        self.norm2 = nn.LayerNorm(n_neurons)

    def forward(self, x):
        x = F.relu(self.norm1(self.fc1(x)))
        x = F.relu(self.norm2(self.fc2(x)))
        output = self.fc3(x)  # No sigmoid
        output = F.softplus(output)  # smooth approximation of ReLU
        return output  # Let output be unbounded
