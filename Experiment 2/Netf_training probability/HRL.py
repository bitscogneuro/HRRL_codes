...
def parseArgs(argv):
    ...
    parser.add_argument('--num_iter', type=int, default=6000,
    ...
    parser.add_argument('--freq_plot', type=int, default=6000,
                        help='Make one plot every freq_plot steps.')

    parser.add_argument('--freq_save_weights', type=int, default=6000,
...
def main(argv):
    ...
    state_dict = torch.load(
        '/Users/ibrain/Downloads/Cognitive Neuroscience Work/boris gutkin/Dec 2025 Revision/Systematic HRRL/Transition_function/weights/weights_net_f',
        map_location=torch.device('cpu'))
    result_f = net_f.load_state_dict(state_dict, strict=False)
    print("net_f loaded:", result_f)
    # #
    # state_dict = torch.load(
    #     '/Users/ibrain/Downloads/Cognitive Neuroscience Work/boris gutkin/Netf/Netf_training/weights/weights_net_J',
    #     map_location=torch.device('cpu'))
    # result_J = net_J.load_state_dict(state_dict, strict=False)
    # print("net_J loaded:", result_J)

    # for name, param in net_f.named_parameters():
    #     print(name, param.data.norm())  # or param.data[0] if it's a small tensor
