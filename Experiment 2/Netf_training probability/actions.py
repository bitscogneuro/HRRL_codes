    ...
    def __init__(self, hyperparam: Hyperparam) -> None:
        ...
        def new_state_and_constraints_walking(direction: str):
            ...
            def new_state_walking(agent: Agent, env: Environment) -> HomogeneousZeta:
                ...
                new_zeta.agent_internal_state = agent.euler_method(agent.zeta, control_walking).agent_internal_state
                return new_zeta

            def constraints_walking(agent: Agent, env: Environment) -> bool:
                new_zeta = new_state_walking(agent, env)  # Get new state after walking

                # Check if current sleep and muscular fatigue are within limits
                current_fatigue_ok = (agent.zeta.sleep_fatigue < self.hp.cst_agent.max_sleep_fatigue) and \
                                     (agent.zeta.muscular_fatigue < self.hp.cst_agent.max_muscular_fatigue)

                # Check if the new position is inside the environment
                position_ok = env.is_point_inside(new_zeta.x, new_zeta.y)

                # The agent can walk only if both current and future fatigue are within limits
                return current_fatigue_ok and position_ok
        ...
        def new_state_sleeping(agent: Agent, env: Environment) -> HomogeneousZeta:
            ...
            new_zeta = HomogeneousZeta(self.hp)
            new_zeta.agent_internal_state = agent.integrate_multiple_steps(
                duration_sleep, agent.zeta, control_sleeping).agent_internal_state
            return new_zeta

        def constraints_sleeping(agent: Agent, env: Environment) -> bool:
            val = agent.zeta.sleep_fatigue
            threshold = self.hp.cst_agent.min_sleep_fatigue
            # print(f"Sleep fatigue: {val}, Threshold: {threshold}, Allowed: {val > threshold}")
        ...
        def new_state_doing_nothing(agent: Agent, env: Environment) -> HomogeneousZeta:
            ...
            new_zeta = HomogeneousZeta(self.hp)
            new_zeta.agent_internal_state = agent.euler_method(agent.zeta, control_doing_nothing).agent_internal_state
            return new_zeta

        def constraints_doing_nothing(agent: Agent, env: Environment) -> bool:
            # drive = agent.drive(agent.zeta)
            sleep_low = agent.zeta.sleep_fatigue < self.hp.cst_agent.max_sleep_fatigue
            # print(f"inside doing nothing constraint {sleep_low} and {drive}")
            return sleep_low
        ...
        def new_state_and_constraints_consuming_resource(res: int):
            if (res < 0) or (res >= self.hp.difficulty.n_resources):
                ...
                def new_state_consuming_resource(agent: Agent, env: Environment) -> HomogeneousZeta:
                    ...
                    new_zeta = HomogeneousZeta(self.hp)
                    new_zeta.agent_internal_state = agent.euler_method(agent.zeta,
                                                                       control_consuming_resource).agent_internal_state
                    return new_zeta
        ...
        def new_state_and_constraints_going_to_resource(res: int):
            if (res < 0) or (res >= self.hp.difficulty.n_resources):
                ...
                def new_state_going_to_resource(agent: Agent, env: Environment) -> HomogeneousZeta:
                    ...
                    new_zeta = HomogeneousZeta(self.hp)
                    new_zeta.agent_internal_state = agent.integrate_multiple_steps(
                        duration_walking, agent.zeta, control_going_to_resource).agent_internal_state
                ...
                def constraints_going_to_resource(agent: Agent, env: Environment) -> bool:
                    ...
                    new_okay = (new_zeta.sleep_fatigue < self.hp.cst_agent.max_sleep_fatigue and
                                new_zeta.muscular_fatigue < self.hp.cst_agent.max_muscular_fatigue)

                    # Ensure the resource is visible and the distance to it is valid
                    resource_okay = env.is_resource_visible(agent.zeta.x, agent.zeta.y, res) and \
                                    (env.distance_to_resource(agent.zeta.x, agent.zeta.y, res) > 0)

                    return new_okay and resource_okay
