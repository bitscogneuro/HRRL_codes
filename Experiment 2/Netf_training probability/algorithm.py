...
class Algorithm:
    ...
    def evaluate_action(self, action: int, is_available: int):
        ...
        if action == 6:
            if not is_available:
                f[0] = 0  # remove predicted gain

        if action == 7:
            if not is_available:
                f[1] = 0  # remove predicted gain
        ...
        true_delta = new_zeta.agent_internal_state - _zeta_tensor

        print(f"grad_ = {grad_filtered}")
        print(f" pred delta is {f}")
    ...
    def simulation_one_step(self, k: int):
        ...
        is_available = np.random.random() <= self.hp.cst_env.resource_availability_prob
        ...
        if np.random.random() <= epsilon:
            ...
            for act in indexes_possible_actions:
                score = self.evaluate_action(act, is_available)
                if score < best_score:
                    best_score = score
                    action = act
        ...
        Loss_f = coeff * torch.sum(diff * diff)
        ...
        print(f"Loss_f {Loss_f} and sign penalty {100000 * torch.sum(sign_penalty)}")
        Loss_f = Loss_f + 10000 * torch.sum(sign_penalty)

        print(f"Loss is {Loss_f}")
        ...
        grad_ = torch.autograd.grad(self.net_J(_zeta), _zeta)[0]
        ...
        delta_deviation = torch.dot(grad_filtered, f)

        discounted_deviation = - torch.log(torch.tensor(self.hp.cst_algo.gamma)) * \
                               self.net_J.forward(_zeta)

        long_term_value = self.net_J.forward(_zeta)

        deviation_fn = self.net_J.forward(_zeta)

        # if k < 2000:
        #     Loss_J = torch.square(self.net_J(_zeta) - instant_drive)
        # else:
        Loss_J = torch.square(
