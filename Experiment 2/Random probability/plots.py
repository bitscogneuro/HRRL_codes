...
class Plots:
    ...
    def plot_resources(self, ax, frame: int, historic_zeta: List[HomogeneousZeta]):
        ...
        historic_zeta_agent_info_vector = [zeta.agent_internal_state.detach(
        ).numpy() for zeta in historic_zeta[:frame+1]]

        df = pd.DataFrame(historic_zeta_agent_info_vector, columns=zeta_meaning)
        ...
        for i in range(n_X):  # x
            for j in range(n_Y):  # y
                if is_inside[i, j]:  # TODO : use torch batch
                    ...
                    with torch.no_grad():
                        values[i, j] = net_J(zeta.agent_internal_state).detach().numpy()
